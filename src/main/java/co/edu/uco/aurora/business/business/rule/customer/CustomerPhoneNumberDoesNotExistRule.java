...
public final class CustomerPhoneNumberDoesNotExistRule implements Rule {
    ...
    public void execute(Object... data) {
        ...
        if (ObjectHelper.isNull(data)){
            var userMessage = MessagesEnumCustomerRule.CUSTOMER_PHONE_NUMBER_DOES_NOT_EXIST_RULE_DATA_IS_NULL.getTitle();
            var technicalMessage = MessagesEnumCustomerRule.CUSTOMER_PHONE_NUMBER_DOES_NOT_EXIST_RULE_DATA_IS_NULL.getContent();
            throw AuroraException.create(userMessage, technicalMessage);
        }
        if (data.length < 2){
            var userMessage = MessagesEnumCustomerRule.CUSTOMER_PHONE_NUMBER_DOES_NOT_EXIST_RULE_DATA_LENGTH_INVALID.getTitle();
            var technicalMessage = MessagesEnumCustomerRule.CUSTOMER_PHONE_NUMBER_DOES_NOT_EXIST_RULE_DATA_LENGTH_INVALID.getContent();
            throw AuroraException.create(userMessage, technicalMessage);
        }
        ...
        if (!UUIDHelper.getUUIDHelper().isDefaultUUID(customer.getId())) {
            ...
            var userMessage = MessagesEnumCustomerRule.CUSTOMER_PHONE_NUMBER_DOES_NOT_EXIST_RULE_CUSTOMER_ALREADY_EXISTS.getTitle();
            var technicalMessage = TextHelper.format(
                    MessagesEnumCustomerRule.CUSTOMER_PHONE_NUMBER_DOES_NOT_EXIST_RULE_CUSTOMER_ALREADY_EXISTS.getContent(),
                    phoneNumber
            );
            throw AuroraException.create(userMessage,technicalMessage);
        }
